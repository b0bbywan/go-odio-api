...
func TestBackendDisabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check Bluetooth
			// In test environment, Bluetooth backend may fail to initialize even when enabled
			// We only verify it's nil when explicitly disabled
			if !tt.bluetoothEnabled && backend.Bluetooth != nil {
				t.Error("Bluetooth should be nil when disabled")
			}
