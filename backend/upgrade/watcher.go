...
// startWatcher watches the result file's parent directory and re-reads on
// change. The directory is watched (not the file) because atomic writes
// (write-temp + rename) replace the inode, which a file watch would miss.
// A watch failure is non-fatal: the initial read and manual checks still work.
