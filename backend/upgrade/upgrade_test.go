...
func TestTrailingLineAfterEndDoesNotResurrectRun(t *testing.T) {
	...
	// One write so all three lines reach the socket buffer before the server reads end and closes
	// the connection: a separate trailing write would race that close and fail with EPIPE.
	mustWrite(t, conn, `{"event":"begin","total":1}`+"\n"+
		`{"event":"end","success":true}`+"\n"+
		`{"event":"progress","percent":50,"current":1,"step":"late"}`)
...
// A CLI run whose connection drops without an end and never reconnects is failed once the grace
// elapses, instead of stranding "running" forever (and wedging the next StartUpgrade).
func TestDisconnectGraceFailsCLIRun(t *testing.T) {
	u := &UpgradeBackend{events: make(chan events.Event, 8), reconnectGrace: 30 * time.Millisecond}
	pct := 40
	u.run.observeProgress(&pct, ptr("mpd")) // CLI run claims the trunk
	recv(t, u.Events(), time.Second)        // drain the running announce

	u.armGrace() // the progress connection dropped without an end

	e, ok := recv(t, u.Events(), time.Second)
	if !ok || e.Type != events.TypeUpgradeInfo {
		t.Fatalf("want a finished verdict after the grace, got (%q, %v)", e.Type, ok)
	}
	run, _ := e.Data.(RunState)
	if run.State != "finished" || run.Success == nil || *run.Success {
		t.Fatalf("verdict = %+v, want finished success=false", e.Data)
	}
	if run.Error == nil || *run.Error != "upgrade connection lost" {
		t.Fatalf("error = %v, want 'upgrade connection lost'", run.Error)
	}
	if u.run.snapshot() != nil {
		t.Fatalf("run still tracked after the grace, want idle")
	}
}

// A reconnecting progress line cancels a pending disconnect grace, so a run that resumed streaming is
// not failed out from under itself.
func TestReconnectCancelsDisconnectGrace(t *testing.T) {
	u := &UpgradeBackend{events: make(chan events.Event, 8), reconnectGrace: 40 * time.Millisecond}
	pct := 40
	u.run.observeProgress(&pct, ptr("mpd"))
	recv(t, u.Events(), time.Second)

	u.armGrace()                                                                                                    // dropped
	u.applyRunProgress(progressLine{Event: ptr("progress"), Percent: ptr(60), Current: ptr(2), Step: ptr("nginx")}) // reconnect cancels it

	if e, ok := recv(t, u.Events(), 200*time.Millisecond); ok {
		t.Fatalf("grace fired despite a reconnect: emitted %q", e.Type)
	}
	if st := u.run.snapshot(); st == nil || st.State != "running" {
		t.Fatalf("run after reconnect = %+v, want still running", st)
	}
}

// The disconnect grace is CLI-only: a unit run keeps its socket as a mere progress channel, the bus
// finishes it, so a dropped socket must not fail it.
func TestDisconnectGraceIgnoresUnitRun(t *testing.T) {
	u := &UpgradeBackend{events: make(chan events.Event, 8), reconnectGrace: 30 * time.Millisecond, upgradeUnit: "odio-upgrade.service"}
	u.run.trigger()
	u.run.observeProgress(ptr(40), ptr("mpd")) // unit run streams progress over the socket

	u.armGrace() // the socket dropped

	if e, ok := recv(t, u.Events(), 120*time.Millisecond); ok {
		t.Fatalf("grace failed a unit run: emitted %q", e.Type)
	}
	if st, _, unit := u.run.inflight(); st == nil || !unit {
		t.Fatalf("unit run = (%+v, unit=%v), want still tracked and unit-owned", st, unit)
	}
}
...
// A stale/repeated terminal event must not finish a newer, unrelated run: onServiceEvent only acts
// on the transition into a terminal state, and a unit terminal never touches a CLI run anyway.
func TestStaleBusFailedDoesNotFinishNewerRun(t *testing.T) {
	...
	// A new CLI run B starts (out of band, stream-driven).
	pct := 60
	u.run.observeProgress(&pct, ptr("late-run"))
	...
	if st, _, unit := u.run.inflight(); st == nil || unit {
		t.Fatalf("run B = (%+v, unit=%v), want still CLI and running", st, unit)
	}
}

// A second unit run started after a first one finalized must still be finished by its own terminal
// event, even when the unit returns to the same ActiveState with no intervening non-terminal event
// observed (a coalesced or refresh-collapsed "activating"). The live run identity, not the ActiveState
// history, gates finalization — the old prev-state dedup stranded the second run on "running".
func TestSecondUnitRunFinalizesAfterRepeatedTerminalState(t *testing.T) {
	u := &UpgradeBackend{events: make(chan events.Event, 8), upgradeUnit: "odio-upgrade.service"}
	failedEvt := events.Event{
		Type: events.TypeServiceUpdated,
		Data: systemd.Service{Name: "odio-upgrade.service", Scope: systemd.ScopeUser, ActiveState: "failed"},
	...
	// Run A finishes on its failed event.
	u.run.trigger()
	u.onServiceEvent(failedEvt)
	if e, ok := recv(t, u.Events(), time.Second); !ok || e.Type != events.TypeUpgradeInfo {
		t.Fatalf("setup: want A's verdict, got (%q, %v)", e.Type, ok)
	}

	// Run B is triggered; its unit reaches the same terminal state with no activating seen in between.
	u.run.trigger()
	u.onServiceEvent(failedEvt)

	e, ok := recv(t, u.Events(), time.Second)
	if !ok || e.Type != events.TypeUpgradeInfo {
		t.Fatalf("want B's finished verdict, got (%q, %v)", e.Type, ok)
	}
	if run, _ := e.Data.(RunState); run.State != "finished" || run.Success == nil || *run.Success {
		t.Fatalf("verdict = %+v, want finished success=false", e.Data)
	}
	if u.run.snapshot() != nil {
		t.Fatalf("run B still tracked: %+v, want idle", u.run.snapshot())
	}
}
...
// A CLI run is snapshotted on graceful shutdown like a unit run, restored as a stream-driven hint,
...
func TestInFlightCLIRunPersistsOnGracefulClose(t *testing.T) {
	...
	pct, step := 40, "upgrade"
	u.run.observeProgress(&pct, &step) // CLI run claims the trunk
	u.Close()
	...
	if u2.resumeHintUnit || u2.resumeHintPhase != phaseRunning {
		t.Fatalf("resume hint = (unit=%v, phase=%v), want (false, running) for a CLI run", u2.resumeHintUnit, u2.resumeHintPhase)
	}
}
...
func TestResumeStreamRunFromSnapshot(t *testing.T) {
	...
	u.resumeHintPhase = phaseRunning // CLI run: resumeHintUnit stays false
	...
	st, _, unit := u.run.inflight()
	if st == nil || unit || st.Percent == nil || *st.Percent != 60 {
		t.Fatalf("inflight = (%+v, unit=%v), want CLI running at 60%%", st, unit)
	}
}
...
func TestResumeRunningWhileUnitActivating(t *testing.T) {
	...
	u.resumeHintPhase, u.resumeHintUnit = phaseRunning, true
	...
	st, _, unit := u.run.inflight()
	if st == nil || !unit || st.Percent == nil || *st.Percent != 40 {
		t.Fatalf("inflight = (%+v, unit=%v), want unit-backed running at 40%%", st, unit)
	}
...
func TestResumeReattachesActivatingUnitWithoutSnapshot(t *testing.T) {
	...
	if st, _, unit := u.run.inflight(); st == nil || !unit {
		t.Fatalf("inflight = (%+v, unit=%v), want a unit-backed run re-attached", st, unit)
	}
}
...
func TestResumeEmitsVerdictWhenUnitFinishedDuringDowntime(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			lr := u.StatusResponse().LastRun
			if tc.success {
				if lr != nil {
					t.Fatalf("last_run = %+v, want none (a success is not kept)", lr)
				}
			} else if lr == nil || lr.Success || lr.Step != "finalize" {
				t.Fatalf("last_run = %+v, want a failure at step finalize", lr)
			}
			if u.run.snapshot() != nil {
...
// Only failures are kept: a successful run records no last_run and clears any prior failure, so a
// successful retry wipes the red badge — and the cleared state survives a restart.
func TestSuccessClearsLastRun(t *testing.T) {
	statePath := filepath.Join(t.TempDir(), "state", "upgrade-run.json")

	u := &UpgradeBackend{events: make(chan events.Event, 8), stateFile: statePath}
	// A failure is recorded.
	u.applyRunProgress(progressLine{Event: ptr("begin"), Total: ptr(1)})
	u.applyRunProgress(progressLine{Event: ptr("end"), Success: ptr(false), Error: ptr("boom")})
	if lr := u.StatusResponse().LastRun; lr == nil || lr.Success {
		t.Fatalf("last_run after failure = %+v, want a failure", lr)
	}
	...
	u.applyRunProgress(progressLine{Event: ptr("end"), Success: ptr(true)})
	if lr := u.StatusResponse().LastRun; lr != nil {
		t.Fatalf("last_run after success = %+v, want none (only failures are kept)", lr)
	}

	// The cleared state is on disk: a fresh backend reloads nothing.
	u2 := &UpgradeBackend{events: make(chan events.Event, 8), stateFile: statePath}
	u2.readState()
	if lr := u2.StatusResponse().LastRun; lr != nil {
		t.Fatalf("reloaded last_run = %+v, want none", lr)
	}
}
...
func TestInFlightUnitRunPersistsOnGracefulClose(t *testing.T) {
	...
	u.run.trigger()
	pct, step := 42, "mpd"
	u.run.observeProgress(&pct, &step)
	u.Close()
...
// StartUpgrade triggers the upgrade unit, claims the run as unit-backed (so the job result, not the
...
func TestStartUpgradeTriggersUnitAndAnnouncesRunning(t *testing.T) {
	...
	if st, _, unit := u.run.inflight(); st == nil || !unit {
		t.Fatalf("inflight = (%+v, unit=%v), want unit-backed", st, unit)
	}
}
