...
// A stream run whose connection drops without an end line is failed, not left stuck 'running':
// no further line is coming to finish it.
func TestStreamRunFailedOnDisconnectWithoutEnd(t *testing.T) {
	u, sock := newSocketBackend(t)
	conn, err := net.Dial("unix", sock)
	if err != nil {
		t.Fatalf("dial: %v", err)
	...
	if e, ok := recv(t, u.Events(), 2*time.Second); !ok || e.Type != events.TypeUpgradeInfo {
		t.Fatalf("setup: want running, got (%q, %v)", e.Type, ok)
	}
	if err := conn.Close(); err != nil { // drop the connection without sending end
		t.Fatalf("close: %v", err)
	}

	v := waitFinished(t, u.Events(), 2*time.Second)
	if v.Success == nil || *v.Success {
		t.Fatalf("verdict = %+v, want failure on disconnect", v)
	}
	if u.run.snapshot() != nil {
		t.Fatalf("run still tracked after disconnect: %+v, want idle", u.run.snapshot())
	}
}
...
// A CLI/stream run has no queryable anchor across a restart, so it is not snapshotted on
// shutdown: persisting it would let resume fabricate a false verdict for a run that may still be
// alive. It resumes on the script's next progress line instead; one ended during downtime is missed.
func TestInFlightCLIRunNotPersisted(t *testing.T) {
	...
	u2.readState()
	if u2.resumeHint != nil {
		t.Fatalf("resumeHint = %+v, want nil (CLI runs are not snapshotted)", u2.resumeHint)
	}
}
