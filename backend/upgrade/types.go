...
type persistedState struct {
	...
	InFlightPhase runPhase  `json:"in_flight_phase,omitempty"` // phaseTriggered | phaseRunning | phaseSettling
...
type UpgradeBackend struct {
	...
	stateFile      string
	reconnectGrace time.Duration // a disconnected CLI run is failed if the script does not reconnect within this
	...
	// graceTimer fails a disconnected CLI run that never reconnects. graceGen invalidates a timer
	// that already fired before a reconnect or finalize could stop it.
	graceMu    sync.Mutex
