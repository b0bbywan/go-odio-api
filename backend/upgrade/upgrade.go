...
func (u *UpgradeBackend) readState() {
	...
	u.resumeHintPhase = phaseFromName(ps.InFlightPhase)
...
func (u *UpgradeBackend) persistInFlight() {
	...
	u.persist(persistedState{LastRun: u.lastRun.Load(), InFlight: st, InFlightPhase: phaseName(phase), InFlightUnit: unit})
}
...
// onServiceEvent emits the run verdict once the upgrade unit reaches a terminal state. The job
// result is authoritative for a unit run — it covers a script killed before its end line. A CLI run
// is untouched: its lifecycle is the progress stream alone.
func (u *UpgradeBackend) onServiceEvent(e events.Event) {
	...
	// unit was already terminal) must not finalize a newer unit run started right after.
	if prev == "active" || prev == "inactive" || prev == "failed" {
		return
	}
	if lr := u.run.observeUnitTerminal(svc.ActiveState != "failed"); lr != nil {
