...
func (u *UpgradeBackend) readState() {
	...
	u.resumeHintSource = sourceFromName(ps.InFlightSource)
}
...
// the right percent. This covers the self-upgrade restart for both sources: a unit run resumes
// against its still-queryable unit, a CLI run resumes blind and waits for the script — which
// reconnects and resends its tail (notably end) — to land on the live run.
func (u *UpgradeBackend) persistInFlight() {
	...
	u.persist(persistedState{LastRun: u.lastRun.Load(), InFlight: st, InFlightSource: sourceName(src)})
}
...
func (u *UpgradeBackend) resumeIfRunning() {
	hint := u.resumeHint
	src := u.resumeHintSource
	u.resumeHint = nil

	// A CLI run has no unit to query: resume it blind from the snapshot and wait for the script to
	// reconnect and resend its tail (progress, then end). It re-claims via progress even without a hint.
	if src == sourceStream {
		if hint != nil && u.run.resume(sourceStream, hint) {
