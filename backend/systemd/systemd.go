...
func (s *SystemdBackend) RefreshService(ctx context.Context, name string, scope UnitScope) (*Service, error) {
	...
	// URL, Panel and Internal are config-derived, not D-Bus-derived, so
	// serviceFromProps can't know about them. Without this lookup, every
	// refresh wipes them.
	cfg := s.configured(name, scope)
	svc.URL, svc.Panel, svc.Internal = cfg.URL, cfg.Panel, cfg.Internal
