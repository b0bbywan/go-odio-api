...
func TestConfigured(t *testing.T) {
	backend := &SystemdBackend{
		config: &config.SystemdConfig{
			...
			UserServices: []config.SystemdService{
				{Name: "snapclient.service", URL: ":1780", Open: config.OpenPanel},
	...
	cases := []struct {
		...
		{"panel service", "snapclient.service", ScopeUser, config.SystemdService{Name: "snapclient.service", URL: ":1780", Open: config.OpenPanel}},
...
func TestServiceJSONOpen(t *testing.T) {
	cases := []struct {
		svc  Service
		want string
	}{
		{Service{Name: "snapclient.service", URL: ":1780", Open: config.OpenPanel}, `"open":"panel"`},
	...
	for _, tc := range cases {
		...
		if tc.want == "" && strings.Contains(string(b), "open") {
			t.Errorf("%s: %s should omit open", tc.svc.Name, b)
		}
	}
}
