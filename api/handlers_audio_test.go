...
// TestHandleBluetoothError tests the Bluetooth error handling function
func TestHandleBluetoothError(t *testing.T) {
	tests := []struct {
		name           string
		err            error
		wantStatusCode int
...
// TestWithBluetoothAction tests the Bluetooth action middleware
func TestWithBluetoothAction(t *testing.T) {
	tests := []struct {
		name           string
		action         func() error
		wantStatusCode int
		...
		{
			name: "successful action returns 202 Accepted",
			action: func() error {
				return nil
			},
			wantStatusCode: http.StatusAccepted,
		...
		{
			name: "failed action returns 500 Internal Server Error",
			action: func() error {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			handler := withBluetoothAction(tt.action)

			req := httptest.NewRequest("POST", "/bluetooth/connect", nil)
			w := httptest.NewRecorder()
