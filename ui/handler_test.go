...
func TestSystemdUnitTemplate_URLLink(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			view:    ServiceView{Name: "snapclient.service", Description: "Snapcast client", URL: ":1780", Panel: true},
			wantSub: `onclick="openServicePanel(':1780', 'Snapcast client'); return false;"`,
		},
		{
			name:    "panel titled by name without description",
			view:    ServiceView{Name: "snapclient.service", URL: ":1780", Panel: true},
...
func TestServicePanelOutsideSSESections(t *testing.T) {
	tmpl := LoadTemplates()
	services := []ServiceView{{Name: "snapclient.service", URL: ":1780", Panel: true}}
...
func TestConvertServices(t *testing.T) {
	tests := []struct {
		...
		{
			name: "service with panel",
			input: []Service{
				{Name: "snapclient.service", Scope: "user", URL: ":1780", Panel: true},
			},
			expected: []ServiceView{
				{Name: "snapclient.service", IsUser: true, URL: ":1780", Panel: true},
			},
		},
		{
