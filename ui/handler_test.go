...
func TestConvertBluetooth(t *testing.T) {
	...
	t.Run("sorts connected first, then paired, then by label", func(t *testing.T) {
		got := convertBluetooth(&BluetoothStatus{
			Powered: true,
			KnownDevices: []BluetoothDevice{
				{Address: "AA:BB:CC:DD:EE:03", Name: "Zeta"},                 // unpaired
				{Address: "AA:BB:CC:DD:EE:02", Name: "Bose", Paired: true},   // paired
				{Address: "AA:BB:CC:DD:EE:01", Name: "JBL", Connected: true}, // connected
			},
		})
		want := []string{"JBL", "Bose", "Zeta"}
		for i, w := range want {
...
func TestBluetoothDevicesTemplate(t *testing.T) {
	...
	view := &BluetoothView{
		...
		Devices: []BluetoothDevice{
			...
			{Address: "2C:41:A1:BD:D1:45", Name: "Bose Solo 5", Paired: true},
