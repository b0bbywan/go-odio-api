...
func TestUpgradeBadgeFailed(t *testing.T) {
	...
	// Once up to date (no upgrade available), a past failure is moot: no permanent red badge.
	if up := render(&UpgradeStatus{Current: "1.0", Latest: "1.0", UpgradeAvailable: false, CanCheck: true, LastRun: failed}); strings.Contains(up, alertPath) {
		t.Errorf("an up-to-date system should not show a stale failure, got: %s", up)
	}
	// A failure with no detection result must not surface an alert (nor a retry to an empty version).
	if nr := render(&UpgradeStatus{CanUpgrade: true, LastRun: failed}); strings.Contains(nr, alertPath) || strings.Contains(nr, "Retry upgrade to ?") {
		t.Errorf("a failure without a detection result must not show an alert/empty retry, got: %s", nr)
	}
}
