...
// Failed reports whether the most recent run failed and is still relevant: no run is in flight
// and an upgrade is still pending. Once the system is up to date (or no detection has run yet) a
// past failure is moot — surfacing it would leave a permanent red badge with no way to dismiss it,
// and since UpgradeAvailable implies a known Latest, the retry action always has a version to show.
func (u *UpgradeStatus) Failed() bool {
	return u != nil && u.LastRun != nil && !u.LastRun.Success && !u.Running() && u.UpgradeAvailable
}
